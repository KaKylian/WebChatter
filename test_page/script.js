...
document.getElementById('dropImage').addEventListener("click", () => document.getElementById('fileImg').click());
document.getElementById('clearimgbtn').addEventListener("click", () => clearMedia('image'));
document.getElementById('dropVideo').addEventListener("click", () => document.getElementById('fileVid').click());
document.getElementById('clearvidbtn').addEventListener("click", () => clearMedia('video'));

document.getElementById('ctrlVitesse').addEventListener("input", function() {
    document.getElementById('valVitesse').innerText = parseFloat(this.value).toFixed(1);
});
document.getElementById('ctrlVolume').addEventListener("input", function() {
    document.getElementById('valVolume').innerText = this.value;
});

document.querySelector('.btn-lancer').addEventListener("click", lancerFlashDepuisControl);
document.querySelector('.btn-stop').addEventListener("click", resetUI);
...
function lancerFlashDepuisControl() {
    ...
    wf.config.texte = document.getElementById('ctrlTexte').value;
    wf.config.imageSrc = pendingImageURL;
    wf.config.video.src = pendingVideoURL;

    const dureeVal = document.getElementById('ctrlDuree').value.trim().toLowerCase();
    wf.config.duree = (dureeVal === 'auto' || dureeVal === 'inf') ? dureeVal : (parseFloat(dureeVal) || 'auto');

    wf.config.video.time    = parseFloat(document.getElementById('ctrlTime').value) || 0;
    wf.config.video.vitesse = parseFloat(document.getElementById('ctrlVitesse').value) || 1;
    wf.config.video.volume  = parseFloat(document.getElementById('ctrlVolume').value);
    wf.config.video.isAudio = document.getElementById('ctrlAudioSeul').checked;

    if (!wf.config.video.src && !wf.config.imageSrc && !wf.config.texte) {
...
document.querySelector('.btn-lancer').addEventListener("click", runPlayground);
document.querySelector('.btn-clear-playground').addEventListener("click", clearPlayground);
